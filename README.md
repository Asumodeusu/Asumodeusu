👋 Приветствую всех! Меня зовут Илья — фронтенд-разработчик.

![Frontend-JS](https://img.shields.io/badge/Frontend-JS-F7DF1E?logo=javascript)
![React](https://img.shields.io/badge/React-20232A?logo=react&logoColor=61DAFB)
![TS](https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind-06B6D4?logo=tailwind-css&logoColor=fff)
![Redux Toolkit](https://img.shields.io/badge/Redux_Toolkit-764ABC?logo=redux&logoColor=white)
![i18next](https://img.shields.io/badge/i18next-26A69A?logo=i18next&logoColor=white)
![Bun](https://img.shields.io/badge/Bun-000?logo=bun)
![Storybook](https://img.shields.io/badge/Storybook-FF4785?logo=storybook&logoColor=fff)
![ESLint](https://img.shields.io/badge/ESLint-4B32C3?logo=eslint&logoColor=fff)


До IT работал в нефтегазовой сфере: управлял проектами, анализировал данные, координировал команды. Начал с Python для расчётов геомеханических моделей — это подтолкнуло к переходу во фронтенд-разработку. Теперь создаю интерфейсы, которыми удобно пользоваться.

👨‍💻 **Ключевое:**
- Разрабатываю React-приложения с нуля (банк, full-stack на Bun)
- Выполнял роль Team Lead для команды из 10 разработчиков — декомпозировал задачи, проводил код-ревью, синхронизировал работу
- Пишу тесты (Jest, Bun), настраиваю линтеры и Storybook

🚀 **Проекты:**
- [Money Bank](https://github.com/Asumodeusu/Money-bank) — мобильное банковское приложение с курсами валют и картой отделений 
Демо - [`money-bank-m.vercel.app`](https://money-bank-m.vercel.app/)
- [API-FRONT](https://github.com/Asumodeusu/API-FRONT) — full-stack приложение на Bun/Elysia для бронирования
- [AutoTest](https://github.com/Asumodeusu/AutoTest) — 20+ unit-тестов на Bun (калькулятор, FizzBuzz, Stack)

📬 **Контакты:**
- Telegram: [@IlyaJSR](https://t.me/IlyaJSR)
- Почта: ishergin@mail.ru
- GitHub: [github.com/Asumodeusu](https://github.com/Asumodeusu)
- Резюме: [hh.ru](https://tyumen.hh.ru/resume/7ccbdc87ff0af810ef0039ed1f413678366775)

📌 **Статус:**

✅ Открыт к предложениям (рассматриваю full-time, part-time, проекты, стажировки)

🌍 Готов к удалённой работе и редким командировкам

📚 Постоянно учусь и осваиваю новые технологии
